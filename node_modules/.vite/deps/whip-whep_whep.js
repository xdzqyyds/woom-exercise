...
// node_modules/whip-whep/whep.js
var Extensions = {
  Core: {
...
var WHEPClient = class extends EventTarget {
  ...
  async view(pc, url, token) {
    ...
    if (this.eventsUrl) {
      const events = links[Extensions.Core.ServerSentEvents][0].params.events ? links[Extensions.Core.ServerSentEvents][0].params.events.split(",") : ["active", "inactive", "layers", "viewercount"];
      const headers2 = {
  ...
  async restart() {
    ...
    this.restartIce = /* @__PURE__ */ new Date();
    this.iceTrickeTimeout = clearTimeout(this.iceTrickeTimeout);
    return this.patch();
  }
  async patch() {
    ...
    if (restartIce && fetched.status === 200) {
      ...
      const candidates2 = Array.from(answer.matchAll(/(a=candidate:.*\r\n)/gm)).map((res) => res[1]);
...
//# sourceMappingURL=whip-whep_whep.js.map
